    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (!busy && sample_valid) begin
                ...
                if (tap_index == KERNEL_TAPS - 1) begin
                    sample_out <= acc + (tap[tap_index] * tap_weight(tap_index));
                    output_valid <= 1'b1;
                    busy <= 1'b0;
                    tap_index <= 16'd0;
                end else begin
                    acc <= acc + (tap[tap_index] * tap_weight(tap_index));
                    tap_index <= tap_index + 16'd1;
                end
            end
        end
    end
