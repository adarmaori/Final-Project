...
def parse-int-list [value: string] {
  $value | split row "," | each {|item| $item | str trim | into int}
}
...
def summarize-report [summary_path: string] {
...

  {
    ...
    sample_width: (get-kv $lines "sample_width" | into int),
    weight_width: (get-kv $lines "weight_width" | into int),
    acc_width: (get-kv $lines "acc_width" | into int),
    kernel_taps: (get-kv $lines "kernel_taps" | into int),
...
def main [
  --pattern: string = "conv1d_large_*.v",
  --out-dir (-o): string = "large_conv_reports",
  --csv: string = "large_conv_reports/summary.csv",
  --samples: int = 1000,
  --sample-widths: string = "8",
  --weight-widths: string = "8",
  --acc-widths: string = "32",
  --kernel-taps: int = 32,
  --freq: int = 25,
  --device: string = "--hx1k",
  ...
  let conv_files = (
    ...
    | where {|path|
        ...
        ($basename != "conv1d_large_benchmark_top.v") and ($basename != "conv1d_large_engine.v")
      }
    | sort
  )

  if (($conv_files | length) == 0) {
    error make {msg: $"No large convolution files matched pattern: ($pattern)"}
  }

  mut rows = []
  let sample_width_values = (parse-int-list $sample_widths)
  let weight_width_values = (parse-int-list $weight_widths)
  let acc_width_values = (parse-int-list $acc_widths)

  for conv_file in $conv_files {
    let module_name = ($conv_file | path parse | get stem)
    for sample_width in $sample_width_values {
      for weight_width in $weight_width_values {
        for acc_width in $acc_width_values {
          let quant_name = $"s($sample_width)_w($weight_width)_a($acc_width)"
          print $"==== Benchmarking ($module_name), K=($kernel_taps), ($quant_name), from ($conv_file) ===="

          ^nu extract_large_conv_reports.nu $conv_file $module_name --out-dir $out_dir --samples $samples --sample-width $sample_width --weight-width $weight_width --acc-width $acc_width --kernel-taps $kernel_taps --freq $freq --device $device --package $package

          let summary_path = ($out_dir | path join $"($module_name)_k($kernel_taps)_($quant_name)" | path join "summary.txt")
